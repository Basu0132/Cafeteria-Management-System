...
if(isset($_POST['submit'])){
   ...
   if(!empty($name)){
      $update_name = $conn->prepare("UPDATE `customer` SET name = ? WHERE id = ?");
   ...
   if(!empty($email)){
      ...
      if($select_email->rowCount() > 0){
         ...
         $update_email = $conn->prepare("UPDATE `customer` SET email = ? WHERE id = ?");
   ...
   if(!empty($number)){
      ...
      if($select_number->rowCount() > 0){
         ...
         $update_number = $conn->prepare("UPDATE `customers` SET number = ? WHERE id = ?");
         $update_number->execute([$number, $user_id]);
      }
   }
   
   $empty_pass = 'da39a3ee5e6b4b0d3255bfef95601890afd80709';
   $select_prev_pass = $conn->prepare("SELECT password FROM `customer` WHERE id = ?");
   ...
   if($old_pass != $empty_pass){
      if($old_pass != $prev_pass){
         ...
         if($new_pass != $empty_pass){
            $update_pass = $conn->prepare("UPDATE `customer` SET password = ? WHERE id = ?");
...
<section class="form-container update-form">
   ...
   <form action="" method="post">
      ...
      <select name="customer_type" required placeholder="Select your customer type" class="box">
